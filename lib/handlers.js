                    ...
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, {'Error': 'Errors encountered while attempting to remove deleted users checks, some of the users checks may still be in the system.'});
                        }
                      }
                    });
                  });
...
// check to make sure id is valid
  if (id) {
    // chekc to make sure one or more optional files have been sent
    if (protocol || url || method || successCodes || timeoutSeconds) {
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
          const token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if(!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      callback(500, {'Error': 'Could not find check on user object, could not remove it'});
                    }


                  } else {
                    callback(500, {'Error': 'Could not find user who created the check, could not remove the check.'});
                  }
                });
              } else {
